                ...
                return Response(
                    {
                        "status": True,
                        "msg": "login sucess",
                        "data": {"token": "Token " + token[0].key},
                    },
                    headers={"Authorization": token[0].key},
                )
            else:
...
class CheckUser(APIView):
    def get(
        self, request
    ):  # used for checking the username in database for new user
        return Response(
            "Restricted", content_type="application/json", status=401
        )
    ...
    def post(self, request):
        if "check_user" in request.POST and request.POST["check_user"] == "1":
            if "user_name" in request.POST and request.POST["user_name"]:
                ...
                if user:
                    return HttpResponse(
                        "Failure", content_type="application/json"
                    )
                else:
                    return HttpResponse(
                        "success", content_type="application/json"
                    )
...
class CreateUser(APIView):
    ...
    def post(
        self, request
    ):  # function is used to create new user from online form
        user = UserCreateSerializer(data=request.data)
        if user.is_valid():
            if (
                "password" in request.data
                and len(request.data["password"]) > 7
            ):
                user = user.save()
                ...
                token = Token.objects.get_or_create(user=user)
                return Response(
                    {"status": True, "msg": "login sucess"},
                    headers={"Authorization": token},
                )

            else:
                Response(
                    {"status": False, "msg": "invalid password"},
...
class DetailsDisplay(APIView):
    def get(self, request):
        ...
        print(type(s1))
        return HttpResponse(
            json.dumps(s1.data), content_type="application/json"
        )
